...
public class HighscoreV2 {
    ...
    private Path highscoreFile = Paths.get("src/logik/HighscoreV2.csv");
    ...
    private String[] gametypes = {"Classic", "RobotV1", "RobotV2", "RobotV3", "RobotV4"};
    ...
    public void writeHighscore(int score){
        ...
        if((getHighscoreValues(currentGametype).stream().filter(a -> a < score).count()) > 0) {
            //if this is greater than 0 this means that the current score is better than the previous ones
            try {
                //first save the whole file
                highscoreReader = Files.newBufferedReader(highscoreFile);
                ...
                //now check which lines we have to replace
                ...
                //now write all of the lines again to the file
                highscoreWriter = Files.newBufferedWriter(highscoreFile, StandardOpenOption.WRITE);
    ...
    private void checkForHighscoreFile() {
        try {
            if (!Files.exists(highscoreFile)) {
                ...
                for(int counter = 0; counter < gametypes.length; counter++){
                    for(int gameTypeMultiplier = 0; gameTypeMultiplier < 3; gameTypeMultiplier++) {
                        highscoreWriter.write(gametypes[counter] + ";" + "000\n");
                    }
                }
                highscoreWriter.close();
            }
